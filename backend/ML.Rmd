...
# Perform the stratified sampling for training set (30%)
train_data <- hdb_data %>%
  group_by(across(all_of(strata_cols))) %>%
  sample_frac(0.3) %>% # Adjust the fraction as needed for training set
  ungroup()
...
### Bagging
```{r}
bagfit = randomForest(log_price~.,data=train_ml,ntree=500,maxnodes=10,mtry=138)
...
for (ntree in ntree_values) {
  for (depth in interaction_depth_values) {
    ...
    mse_cv <- gbm_model$cv.error[gbm_model$best.t]
...
### Stuff to discuss whether to delete
